...
class DomainCrawler:
    ...
    def run(self) -> List[str]: # MODIFIED: Returns list of output filenames
        ...
        all_properties_buffer = [] # ADDED: Buffer for all properties for combined file
        output_mode = CONFIG.get('output', {}).get('mode', 'per_url') # ADDED: Get output mode
        combined_prefix = CONFIG.get('output', {}).get('combined_prefix', 'Combined') # ADDED: Get combined prefix
        ...
        try:
            ...
            for i_url, url in enumerate(urls, 1):
                try:
                    ...
                    # Extract region name and decide on flushing
                    ...
                    if CONFIG['features']['enable_batch_write']:
                        if output_mode in ['per_url', 'hybrid']:
                            # For modes that save per URL, we need to clone the buffer before flushing
                            # because flush will clear it.
                            data_to_flush = list(self.batch_writer.buffer)
                            ...
                            # After flushing, restore the buffer if we are also combining
                            if output_mode == 'hybrid':
                                self.batch_writer.buffer = data_to_flush
                        
                        if output_mode in ['single_file', 'hybrid']:
                            all_properties_buffer.extend(self.batch_writer.buffer)
                            self.batch_writer.buffer = [] # Clear buffer after moving data
            ...
            # ADDED: Final flush for 'single_file' or 'hybrid' mode
            if CONFIG['features']['enable_batch_write'] and output_mode in ['single_file', 'hybrid'] and all_properties_buffer:
                ...
                # Temporarily set the main buffer to the combined data for flushing
                ...
                if combined_file:
                    ...
                    logger.info(f"所有房源数据已合并保存到: {combined_file}")
                else:
                    logger.warning("合并文件保存失败或无数据可保存")
