...
class PropertyData:
    ...
    features: PropertyFeatures = field(default_factory=PropertyFeatures) # Will be initialized with config later
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {}
        # Manually iterate to control order and handle nested dataclass
        # __dataclass_fields__ is a dict, iterating it yields field names (strings)
        for field_name in self.__dataclass_fields__:
            field_value = getattr(self, field_name)
            if field_name == 'features':
                result.update(field_value.to_dict())
            elif isinstance(field_value, list):
                result[field_name] = '; '.join(map(str, field_value))
            else:
                result[field_name] = field_value
        return result

def get_expected_columns(features_config: Optional[Dict[str, Any]] = None) -> List[str]:
    """Generates the list of expected columns dynamically."""
    base_columns = [
        'listing_id', 'property_url', 'address', 'suburb', 'state', 'postcode',
        'property_type', 'rent_pw', 'bond', 'bedrooms', 'bathrooms', 'parking_spaces',
        'bedroom_display', 'available_date', 'inspection_times', 'agency_name', 'agent_name', 
        'cover_image', 'agent_phone', 'agent_email', 'property_headline', 'property_description'
    ]
    
    feature_columns = []
    # Add legacy feature columns first to maintain order
    feature_columns.extend(['furnishing_status', 'has_air_conditioning', 'air_conditioning_type'])
    ...
    else: # Fallback to old hardcoded features if config is missing
        fallback_features = [
            'has_balcony', 'has_dishwasher', 'has_laundry', 'has_built_in_wardrobe', 
            'has_gym', 'has_pool', 'has_parking', 'allows_pets', 'has_security_system', 
            'has_storage', 'has_study_room', 'has_garden'
        ]
    ...
    return base_columns + feature_columns + remaining_columns

# =============================================================================
# 特征提取, 数据清洗 & 验证 (MODIFIED for dynamic features)
# =============================================================================
class FeatureExtractor:
    """从房源文本中提取各种Features。"""
    def __init__(self, features_config: Dict[str, Any]):
        self.features_config = features_config
        # Legacy keywords can still be used for furnishing and AC status for backward compatibility
        # or as a fallback.
        self.legacy_furnish_keywords = {
            'furnished': ['furnished', 'fully furnished', 'incl. furniture'],
            'partly furnished': ['partly furnished', 'semi-furnished'],
            'unfurnished': ['unfurnished'],
        }
        self.legacy_ac_keywords = {
            ...
            for feature_config in self.features_config['features']:
                ...
                if column_name and keywords and hasattr(features, column_name):
                    for keyword in keywords:
                        if keyword.lower() in text_blob_lower:
                            setattr(features, column_name, True)
                            break # Move to next feature once one keyword matches

        # --- Legacy Feature Extraction (for backward compatibility) ---
        self._extract_legacy_features(features, text_blob_lower)
    ...
    def _extract_legacy_features(self, features: PropertyFeatures, text_blob: str):
        """提取旧版的空调和家具信息以保证后向兼容。"""
        # Furnishing Status
        if any(keyword in text_blob for keyword in self.legacy_furnish_keywords['furnished']):
            features.furnishing_status = 'furnished'
        elif any(keyword in text_blob for keyword in self.legacy_furnish_keywords['partly furnished']):
            features.furnishing_status = 'partly_furnished'
        
        # Air Conditioning
        ac_found = False
        if any(keyword in text_blob for keyword in self.legacy_ac_keywords['split system']):
            features.air_conditioning_type = 'split_system'
            ac_found = True
        elif any(keyword in text_blob for keyword in self.legacy_ac_keywords['ducted']):
            features.air_conditioning_type = 'ducted'
            ac_found = True
        elif any(keyword in text_blob for keyword in self.legacy_ac_keywords['air conditioning']):
            features.air_conditioning_type = 'yes' # Generic 'yes'
            ac_found = True
...
# =============================================================================
# 批量写入管理 (MODIFIED for dynamic columns)
# =============================================================================
class BatchWriter:
    def __init__(self, expected_columns: List[str]):
        self.buffer: List[PropertyData] = []
    ...
    def flush(self, region: str = "Unknown", total_count: int = 0) -> Optional[str]:
        if not self.buffer:
            ...
            return None
        with self._lock:
            output_filename = "unknown_file"
            try:
                data_dicts = [item.to_dict() for item in self.buffer]
                temp_df = pd.DataFrame(data_dicts)
                
                # Reorder and fill missing columns dynamically
                df_final = pd.DataFrame(columns=self.expected_columns)
                for col in self.expected_columns:
                    if col in temp_df.columns:
                        df_final[col] = temp_df[col]
                    else:
                        # This handles cases where a feature column might not be present in any scraped item
                        df_final[col] = "" 
                ...
                logger.error(f"写入XLSX文件 ({output_filename}) 失败: {e}", exc_info=True)
                return None

# =============================================================================
# 爬虫核心 (MODIFIED to accept config and use dynamic components)
# =============================================================================
class DomainCrawler:
    def __init__(self, features_config: Dict[str, Any]):
        self.features_config = features_config
        self.request_manager = RequestManager()
        self.feature_extractor = FeatureExtractor(self.features_config)
        self.data_cleaner = DataCleaner()
        self.data_validator = DataValidator()
        self.expected_columns = get_expected_columns(self.features_config)
        self.batch_writer = BatchWriter(self.expected_columns)
        self._lock = threading.Lock()
    ...
    def crawl_detail(self, house_href: str) -> Optional[PropertyData]:
        try:
            ...
            # Create the combined text blob for the new feature extractor
            text_blob = (root_q.get("description", "") or "") + " " + " ".join(prop_feat_list)
        ...
        try:
            ...
            for i_url, url in enumerate(urls, 1):
                try:
                    ...
                    # REMOVED faulty logic. All URLs are processed as search URLs for consistency.
                    logger.info(f"Processing as search URL (from {url_cfg_path.name}): {url}")
...
def load_features_config(config_path_str: str) -> Dict[str, Any]:
    """加载Features配置文件。"""
    features_config_path = Path(config_path_str)
    if not features_config_path.exists():
        logger.error(f"Features configuration file not found: {features_config_path}")
        return {}
    try:
...
if __name__ == "__main__":
    logger.info("加载动态特征配置文件...")
    features_config = load_features_config('config/features_config.yaml')
    
    # Pass the loaded config to the crawler
    crawler = DomainCrawler(features_config=features_config)
